...
dadosT <- dados %>% mutate(alfab = ifelse(ProfComb <= 95, 0, 1)) %>% 
  ...
  mutate(continuaEstudo = ifelse(p3 == 1, 1, 0)) %>% 
  filter(p1 == 9) %>% 
  filter(p501 != 2) %>%
...
vars_paream <- c('id_real', 'sexoT', 'idadeConclusao', 'racaT', 'maeMedioCompleto')
...
AutogestaoSemMissing <- dadosW %>% 
  select(ProfComb, Autogestao, AberturaNovo, Autoconceito, continuaEstudo, one_of(vars_paream)) %>% 
  na.omit()
...
write.csv(AutogestaoMatchResult, file = paste0(getwd(), '/results/MatchingSummary_Autogestao_EF_II.csv'))

#Estimando os efeitos para HSE e salvando os resultados

didAutogestao <- lm(ProfComb ~ Autogestao + continuaEstudo + Autogestao*continuaEstudo, 
                    data = matchedAutogestao)
write.csv(tidy(didAutogestao), file = paste0(getwd(), '/results/Autogestao_EF_II.csv'))
...
write.csv(AberturaNovoMatchResult, file = paste0(getwd(), '/results/MatchingSummary_AberturaNovo_EF_II.csv'))

#Estimando os efeitos para HSE e salvando os resultados

didAberturaNovo <- lm(ProfComb ~ AberturaNovo + continuaEstudo + AberturaNovo*continuaEstudo, 
                      data = matchedAberturaNovo)
write.csv(tidy(didAberturaNovo), file = paste0(getwd(), '/results/Openess_EF_II.csv'))